...
const ImageFull = ({ isOpen, onClose, imgs, currentImageIndex, setCurrentImageIndex }) => {
  ...
  return (
    <div className="image-overlay" onClick={onClose}>
        <div className="full-image-slider" onClick={(e) => e.stopPropagation()}>
                ...
                <span className="full-img-number">{currentSlide + 1} / {imgs.length}</span>
                <Slider {...modalSliderSettings} className='full-car-card'>
                {imgs.map((img, index) => (
                        <img src={img} alt={`vb-${index}`} className="full-image" key={index} />                       
                ))}
                </Slider>
        </div>   
    </div>
